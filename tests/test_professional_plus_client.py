...
from fixerio.professional_plus_client import ProfessionalPlusClient


class FixerioBasicClientTestCase(unittest.TestCase):

    @patch('fixerio.professional_plus_client.ProfessionalPlusClient._symbols')  # Mocking the _symbols method
    def test_get_symbols_calls_symbols_once_and_returns_value(self, mock_symbols):
        client = ProfessionalPlusClient(access_key='test-access-key')
    ...
    @patch('fixerio.professional_plus_client.ProfessionalPlusClient._latest')  # Mocking the _latest method
    ...
    @patch('fixerio.professional_plus_client.ProfessionalPlusClient._historical_rates')  # Mocking the _historical_rates method
    ...
    @patch('fixerio.professional_plus_client.ProfessionalPlusClient._convert')  # Mocking the _convert method
    ...
    @patch('fixerio.professional_plus_client.ProfessionalPlusClient._time_series')  # Mocking the _time_series method
    ...
    @patch('fixerio.professional_plus_client.ProfessionalPlusClient._fluctuation')  # Mocking the _fluctuation method
    def test_get_fluctuation_series_rates_calls_time_series_once_and_returns_value(self, mock_fluctuation):
        client = ProfessionalPlusClient(access_key='test-access-key')

        start_date = "2001-01-01"
        end_date = "2001-01-02"
        expected_return_value = {"2001-01-01": 1.0}
        mock_fluctuation.return_value = expected_return_value

        result = client.get_fluctuation(start_date, end_date, ["USD"], "USD")

        mock_fluctuation.assert_called_once()

        self.assertEqual(result, expected_return_value)

    def test_inherited_from_cheaper_subscriptions(self):
        self.assertTrue(issubclass(ProfessionalPlusClient, fixerio.free_client.FreeClient))
        self.assertTrue(issubclass(ProfessionalPlusClient, fixerio.basic_client.BasicClient))
        self.assertTrue(issubclass(ProfessionalPlusClient, fixerio.professional_client.ProfessionalClient))
